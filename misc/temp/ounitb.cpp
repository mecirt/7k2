...
#ifdef DEBUG
	pathfind_profile_time += m.get_time() - pathFinderStartTime;
#endif
...
/*		int occUnitNum;
	...
	if(wait_state++ > (MAX_UNIT_WAIT_STATE))
	{
		wait_state = 0;
		move_to(move_to_loc_x, move_to_loc_y);
		retry_state++;
...
int UnitB::checking_who_occupy_the_place_i_want_to_go(int checkingLocX, int checkingLocY)
{
	...
	while(!occupyingUnitRecordNumber && is<obj_loc_width())
	{
		...
		while(!occupyingUnitRecordNumber&& js<obj_loc_height())
		{
			...
			if(occupyingUnitRecordNumber && ((occupyingUnitRecordNumber == sprite_recno) || (theOccupyingStuff = unit_array[occupyingUnitRecordNumber])->nation_recno != nation_recno || theOccupyingStuff->cur_action == SPRITE_MOVE || theOccupyingStuff->cur_action == SPRITE_WAIT || theOccupyingStuff->cur_action == SPRITE_ATTACK))

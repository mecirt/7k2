...
static int select_option2()
{
	...
	for( i = 0; i < 6; ++i )
	{
		if( i < 5 )
			...
			strcpy( diffButtonStr[i], "Custom" );

		w = font_thin_black.text_width(diffButtonStr[i]);
		h = font_thin_black.text_height();
		diffGroup[i].create( cx, cy, cx+w+10, cy+h+10,
			i_disp_text_button, ButtonCustomPara(diffButtonStr[i], i), 0);
		cx += w + 12;
	}
	...
	{
		w = font_thin_black.text_width(terrainButtonStr[i]);
		h = font_thin_black.text_height();
		terrainGroup[i].create( cx, cy, cx+w+10, cy+h+10, 
			i_disp_text_button, ButtonCustomPara(terrainButtonStr[i], i+1), 0 );
		cx += w + 12;
	}
	...
	cx = option3X;
	cy = 99;
	w = font_thin_black.text_width( "Unexplored" );
	h = font_thin_black.text_height();
	exploreGroup[0].create(cx, cy, cx+w+10, cy+h+10,
		i_disp_text_button, ButtonCustomPara( "Unexplored", 0), 0 );
	cx += w+20;
	w = font_thin_black.text_width( "Explored" );
	exploreGroup[1].create(cx, cy, cx+w+10, cy+h+10,
		i_disp_text_button, ButtonCustomPara( "Explored", 1), 0 );
	...
	for( i = 0; i < 4; ++i )
	{
		w = font_thin_black.text_width( treasureSizeStr[i] );
		h = font_thin_black.text_height();
		treasureGroup[i].create(cx, cy, cx+w+10, cy+h+10,
			i_disp_text_button, ButtonCustomPara( treasureSizeStr[i], i+1), 0 );
		cx += w+20;
	}
	...
	for( i = 0; i < 5; ++i )
	{
		w = font_thin_black.text_width( aggressiveStr[i] );
		h = font_thin_black.text_height();
		aiAggressiveGroup[i].create(cx, cy, cx+w+10, cy+h+10,
			i_disp_text_button, ButtonCustomPara( aggressiveStr[i], i), 0 );
	...
	char *spyMethodStr[2] = { "Research Required", "All Available" };
	...
	for( i = 0; i < 2; ++i )
	{
		w = font_thin_black.text_width( yesNoStr[i] );
		h = font_thin_black.text_height();
		randomStartUpGroup[i].create(cx, cy, cx+w+10, cy+h+10,
			i_disp_text_button, ButtonCustomPara(yesNoStr[i], i), 0);
		cx += w+20;
	}
	...
	for( i = 0; i < 3; ++i )
	{
		w = font_thin_black.text_width( fewManyStr[i] );
		h = font_thin_black.text_height();
		townStartGroup[i].create(cx, cy, cx+w+10, cy+h+10,
			i_disp_text_button, ButtonCustomPara(fewManyStr[i], startTownArray[i]), 0);
		cx += w+20;
	}
	...
	for( i = 0; i < 3; ++i )
	{
		w = font_thin_black.text_width( lowHighStr[i] );
		h = font_thin_black.text_height();
		townResistGroup[i].create(cx, cy, cx+w+10, cy+h+10,
			i_disp_text_button, ButtonCustomPara(lowHighStr[i], i+1), 0);
		cx += w+20;
	}
		...
		while(1)
		{
			...
			if( refreshFlag )
			{
				...
				if( optionMode == OPTION_BASIC )
				{
					if( refreshFlag & SGOPTION_PAGE )
					{
						...
						if( tempConfig.race_id > 0 )
							...
							font_bold_black.center_put( 116, 126, 303, 146,	"Fryhtan Species");
						font_bold_black.center_put( 382, 129, 600, 149, "Color" );
						font_bold_black.center_put( 297, 225-20, 698, 243-20, "Computer Controlled Kingdoms" );
						...
						font_bold_black.put( aiMonsterNationGroup[MAX_NATION-1].x2+10, aiMonsterNationGroup[MAX_NATION-1].y1,
							"Fryhtan" );
						font_bold_black.center_put( 341, 305-30, 654, 324-30, "Difficulty Level" );
						font_bold_black.center_put( 370, 380-60, 633, 393-60, "Terrain Type" );
						...
						if (tempConfig.race_id < 0)
						{
							...
							vga.active_buf->put_bitmap_trans( 220 -((Bitmap *)bitmapPtr)->get_width()/2,
								370 -((Bitmap *)bitmapPtr)->get_height()/2, bitmapPtr );
						}
				...
				if( optionMode == OPTION_GOAL )
				{
					if( refreshFlag & SGOPTION_PAGE )
					{
						...
						font_thin_black.put_paragraph( option5X+tickWidth+10, 145, option5X2-10, 178-1, 
							"Defeats All Other Kingdoms" );
						font_thin_black.put_paragraph( option5X+tickWidth+10, 178, option5X2-10, 211-1, 
							"Has Destroyed All Fryhtans" );
						// ######## begin Gilbert 11/3 ##########//
						font_thin_black.put_paragraph( option5X+tickWidth+10, 211, option5X2-10, 244-1, 
							"Defeat All except one Allied Kingdom" );
						font_thin_black.put_paragraph( option5X+tickWidth+10, 244, option5X2-10, 277-1, 
							"Achieves A Population Of" );
						font_thin_black.put_paragraph( option5X+tickWidth+10, 277, option5X2-10, 310-1, 
							"Reaches An Economic Score Of");
						font_thin_black.put_paragraph( option5X+tickWidth+10, 310, option5X2-10, 343-1, 
							"Reaches A Total Score Of");
						font_thin_black.put_paragraph( option5X+tickWidth+10, 343, option5X2-10, 376-1, 
							"Achieves The Selected Within" );
						// ######## end Gilbert 11/3 ##########//
					...
					if( refreshFlag & SGOPTION_TIME_LIMIT )
					{
						...
						font_thin_black.put( yearInc.x1-60, yearInc.y1, "years", 1 );
						yearInc.paint();
						yearDec.paint();
					}
				...
				{
					font_bld.center_put( 40, 85, 82, 105, "Level", 1 );
			...
			else if( optionMode == OPTION_BASIC )
			{
				...
				else if( tempConfig.race_id < 0 && monsterRaceGroup.detect() >= 0 )
				{
					...
					//refreshFlag |= SGOPTION_RACE;
					refreshFlag |= (SGOPTION_PAGE | SGOPTION_RACE | SGOPTION_COLOR | SGOPTION_AI_NATION |
						SGOPTION_DIFFICULTY | SGOPTION_TERRAIN | SGOPTION_BUILDING_SIZE | SGOPTION_NAME_FIELD);
				}

...
char* Unit::unit_name(int withTitle, int firstNameOnly)
{
	...
	if( race_id && unit_id != UNIT_WAGON )
	{
		...
		if( hero_id )
		{
			str += hero_res[hero_id]->name;
		}
		else
		{
			if( rank_id == RANK_KING )		// use the player name
				str += nation_array[nation_recno]->king_name(firstNameOnly);
			else
				str += race_res[race_id]->get_name(name_id, firstNameOnly?1:0);
		}
	}
	else
	{
		str = "";
		if (unitInfo->unit_class == UNIT_CLASS_ANIMAL && attack_count == 2)
		{
		...
		if( unitInfo->class_info.has_weapon_version && get_weapon_version() > 1 )
		// ####### end Gilbert 24/3 ########//
		{
			str += " ";

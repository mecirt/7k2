...
//	if( is_own_spy() && button_spy_item.is_pushed )		// spy menu
	...
	if( refreshFlag == INFO_REPAINT || last_item_menu_offset != item_menu_offset )
	{
		...
		button_use_item.create_text(INFO_X1+127 - item_menu_offset, INFO_Y1+120, INFO_X1+164 - item_menu_offset, INFO_Y1+140, "on" );
	}
...
void Unit::disp_unit_profile(int dispY1, int refreshFlag)
{
	...
	//	if(unit_res[unit_id]->unit_class == UNIT_CLASS_MONSTER && race_id != 0)
	//		str += monster_res[monster_id()]->name;
		...
		case UNIT_CLASS_HUMAN:
			{
				font_whbl.center_put( INFO_X1 +12, INFO_Y1 +21, INFO_X2, INFO_Y1 +33, str, 0, 1 );
		...
		case UNIT_CLASS_CARAVAN:
			{
				str += unit_name(0);
				font_snds.center_put( INFO_X1 +82, INFO_Y1 +15 , INFO_X1 +222, INFO_Y1 +30, str, 0, 1 );
			}
			break;
		...
		case UNIT_CLASS_WAGON:
			{
				...
				font_whbl.center_put( INFO_X1 +12, INFO_Y1 +12, INFO_X2, INFO_Y1 +30, str, 0, 1 );
			}
			break;
		...
		case UNIT_CLASS_ANIMAL:
			{
				font_whbl.center_put( INFO_X1 +12, INFO_Y1 +21, INFO_X2, INFO_Y1 +33, str, 0, 1 );
				str = nation_array[nation_recno]->king_name(0);
				str += "'s ";
				str += unit_name(0);
				font_whbl.center_put( INFO_X1 +12, INFO_Y1 +9, INFO_X2, INFO_Y1 +21, str, 0, 1 );
			}
		...
		case UNIT_CLASS_MONSTER:
			{
				...
				if (rank_id == RANK_SOLDIER)
					str += " Croul";
				font_whbl.center_put( INFO_X1 +12, INFO_Y1 +21, INFO_X2, INFO_Y1 +33, str, 0, 1 );
				font_whbl.center_put( INFO_X1 +12, INFO_Y1 +9, INFO_X2, INFO_Y1 +21, unit_name(rank_id == RANK_SOLDIER ? 0:1), 0, 1 );
			}
			break;
...
void Unit::disp_build(int refreshFlag)
{
	...
	str += " life points";
	font_snds.put_paragraph( INFO_X1+20, INFO_Y1+60, INFO_X1 +216, INFO_Y1 +80, str );
...
//		return;
	...
	if( rank_id != RANK_KING && (nation_recno || spy_recno) && unitInfo->class_info.has_loyalty )
	{
		...
		//	info.disp_loyalty( INFO_X1+20, INFO_Y1+208, INFO_X1+80, spy_array[spy_recno]->spy_loyalty, spy_array[spy_recno]->spy_loyalty, true_nation_recno(), INFO_REPAINT );
			info.disp_loyalty( INFO_X1+20, INFO_Y1+206, INFO_X2-99 - font_snds.text_width(m.format(spy_array[spy_recno]->spy_loyalty, 4)),
		...
		//	info.disp_loyalty( INFO_X1+20, INFO_Y1+208, INFO_X1+80, (int)loyalty, (int)target_loyalty, nation_recno, INFO_REPAINT );
			if ((int)loyalty != (int)target_loyalty)
				info.disp_loyalty( INFO_X1+20, INFO_Y1+206, INFO_X2-99 - font_snds.text_width(m.format((int)loyalty, 4)) -
...
//	if ( race_id )
	...
	{
		x2 = font_snds.put( INFO_X1+130, INFO_Y1+192, "Combat" );
	...
	{
		x2 = font_snds.put( INFO_X1+130, INFO_Y1+206, "Leadership" );
	...
	if( max_hit_points() )
	{
		x2 = font_snds.put( INFO_X1+20, INFO_Y1+192, "Hit Points" );
		int hitPoints;
		if( hit_points > (float)0 && hit_points < (float)1 )
			hitPoints = 1;		// display 1 for value between 0 and 1
		else
		...
		font_snds.right_put( INFO_X2-100, INFO_Y1+192, str );
	}

	if( is_own_spy() )
	{
		x2 = font_snds.put( INFO_X1+130, INFO_Y1+178, "Spying" );
...
void Unit::disp_spy_menu()
{
	...
	{
		font_whbl.put_paragraph( INFO_X1+128, INFO_Y1+104, INFO_X1+217, INFO_Y1+165, "Enemies Nearby !! Cannot change cloak." );
		return;
	}

	//-------------------Display Cloak for selection--------------------------//
	font_whbl.center_put( INFO_X1+158, INFO_Y1+150, INFO_X1+217, INFO_Y1+162, "Spy Cloak:" );
...
//	vga.active_buf->bar( INFO_X1+127, INFO_Y1+90, INFO_X1+127+92-1, INFO_Y1+90+73-1, V_BLACK);
	...
	if (is_own())
		font_whbl.put_paragraph( INFO_X1+127+2 - item_menu_offset, INFO_Y1+153, INFO_X1+218 - item_menu_offset, INFO_Y1+170,
			"Right click to drop" );
	...
	if ( item.can_turn() )
	{
		if( !item.is_turn_on() )
		{
			...
			button_use_item.set_body("off" );
		}
		else
		{
			button_use_item.enable_flag = 1;
			button_use_item.set_body("on" );
		}
...
void Unit::disp_auto_menu(int refreshFlag)
{
	...
	str = headingStr;
	font_bld.center_put_paragraph( INFO_X1+17, INFO_Y1+20, INFO_X2-7, INFO_Y1+70, str, 0, 0, 0 );
...
void Unit::disp_hire_menu(int refreshFlag)
{
	...
	font_snds.put_paragraph( INFO_X1+22, INFO_Y1+18, INFO_X2-10, INFO_Y1+200, str );
...
int Unit::disp_edit_mode(int refreshFlag)
{
	...
	x = font_whbl.put( x1, y, "Hit Points", 0, x2 );
	...
	{
		x = font_whbl.put( x1, y, "Loyalty", 0, x2 );
	...
	if( unitInfo->class_info.has_combat_level )
	{
		x = font_whbl.put( x1, y, "Combat Level", 0, x2 );
	...
	{
		x = font_whbl.put( x1, y, "Leadership", 0, x2 );
	...
	if( spy_recno )
	{
		x = font_whbl.put( x1, y, "Spy Skill", 0, x2 );
		x = edit_spy_x2 = font_blue.put( edit_spy_x1=x+20, edit_spy_y1=y, spy_array[spy_recno]->spy_skill, 0, x2 );
	...
	if( item.id )
	{
		x = font_whbl.put( x1, y, "Item", 0, x2 );
		x = font_whbl.put( x+20, y, item.item_name(), 0, x2 );
		font_whbl.put( x1, y+16, item.item_desc(), 0, x2 );
	}
	y += 36;

	font_blue.put( x1, INFO_Y2-28, "Left click to increase" );
	font_blue.put( x1, INFO_Y2-14, "Right click to decrease" );

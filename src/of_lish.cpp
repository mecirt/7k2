		      ...
		      //--- if there is an enemy unit here ---// 
				...
				if( locPtr->unit_recno(UNIT_LAND) )
				{
					...
					if((((unitPtr->nation_recno != nation_recno) && (firm_id != FIRM_WILDE_LISHORR))
						|| ((unitPtr->nation_recno != nation_recno) && (unitPtr->monster_id()!= MONSTER_KHARSHUF) && (firm_id == FIRM_WILDE_LISHORR)))
				...
				else if( locPtr->is_firm() )
				{
					...
					if ((((firmPtr->nation_recno != nation_recno) && (firm_id != FIRM_WILDE_LISHORR))
						|| ((firmPtr->nation_recno != nation_recno) && (firmPtr->monster_id()!= MONSTER_KHARSHUF) && (firm_id == FIRM_WILDE_LISHORR)))
...
int FirmLishorr::find_best_place_loc(short buildFirmId, short refXLoc, short refYLoc, short& resultXLoc, short& resultYLoc, short distance)
{

...
void Unit::execute_attack()
{
	...
	if( !validate_attack_target() )
	{
		if( has_pushed_order() )
		{
			...
			// ####### begin Gilbert 31/5 ########//
			// if it aggressive attack from a stop action, pushed order become a move back to the original location
			// perform another aggressive attack first before return
			else if( pushed_order.mode == UNIT_STOP )
			{
				...
				else	// proceed move action
				{
					...
					if( pushed_order.para == -2		// see pushed_order
						&& pushed_order.loc_x >= 0 && pushed_order.loc_x < MAX_WORLD_X_LOC
						&& pushed_order.loc_y >= 0 && pushed_order.loc_y < MAX_WORLD_Y_LOC)
					{
...
//				set_attack_dir(curXLoc, curYLoc, range_attack_x_loc, range_attack_y_loc);
				...
				if( unit_id == UNIT_JAPANESE_SPU
					&& attackInfo->consume_power == 0 
					&& nation_recno && tech_res[TECH_SPU_BERSERK(RACE_JAPANESE)]->get_nation_tech_level(nation_recno)
...
//			return;
		...
		if( attackerObj->cast_to_Unit() 
			...
			|| area_distance(attackerObj) <= maxAttackRange		// if attacking unit is out of range but new target is within range
	...
	for( int i=1 ; i<=scanRange*scanRange ; i++ )
	{
		...
		if( baseObjRecno )
		{
			...
			if( baseObj->nation_recno &&
				can_attack_target(baseObjRecno) && 

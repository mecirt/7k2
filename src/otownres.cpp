...
void TownRes::load_town_build()
{
	...
	for( i=0 ; i<town_build_count ; i++ )
	{
		...
		townBuild->loc_width  = m.atoi(townBuildRec->loc_width, TownBuildRec::LOC_LEN);
		townBuild->loc_height = m.atoi(townBuildRec->loc_height, TownBuildRec::LOC_LEN);

		townBuild->offset_x = m.atoi( townBuildRec->offset_x, TownBuildRec::OFFSET_LEN );
		townBuild->offset_y = m.atoi( townBuildRec->offset_y, TownBuildRec::OFFSET_LEN );

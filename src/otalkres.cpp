...
int TalkRes::add_trade_embargo_choices()
{
	...
	choice_question = text_talk.str_ask_embargo(); // "Request an embargo on trade with which kingdom?";
...
int TalkRes::can_send_msg(int toNationRecno, int fromNationRecno, int talkId)
{
	...
	{
		case TALK_PROPOSE_TRADE_TREATY:
			return relationStatus !=RELATION_ALLIANCE && 		// allied nations are oblied to trade with each other
...
//
int TalkRes::ai_send_talk_msg(int toNationRecno, int fromNationRecno, int talkId,
										int talkPara1, int talkPara2, int forceSend, int talkPara3, int talkPara4)
{
	Nation* fromNation = nation_array[fromNationRecno];
	...
	//--- if the AI has been hard-wired to never consider this diplomatic option with the toNationRecno, then do nothing ---//

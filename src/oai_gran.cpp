...
/*
		else
		{
			...
			for( int j=nation_array.size() ; j>0 ; j-- )
			{
				...
				if( get_relation_status(j) != RELATION_HOSTILE )   // only if this is one of our enemies.
					continue;
...
int Nation::think_eliminate_enemy_unit(int enemyNationRecno)
{
	...
	for( int i=unit_array.size() ; i>0 ; i-- )
	{
		...
		return ai_attack_target(unitPtr->next_x_loc(), unitPtr->next_y_loc(), mobileCombatLevel + (int) unitPtr->obj_power());
	}

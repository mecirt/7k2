...
void FirmLair::process_ai()
{
	...
	if( info.game_date%60==(firm_recno+15)%60 || info.game_start_date == info.game_date )		// recruit immediately in the beginning of the game
	{
		if( !think_repress_new_town() )
		{
			...
			if( nationPtr->total_camp_unit_space(0) <=
...
int FirmLair::ai_should_close()
{
	...
	return nationPtr->total_camp_unit_space(0) >= nationPtr->total_unit_count + MAX_SOLDIER &&		// we can eliminate a firm and can still keep all units
			 linked_town_count==0 &&
...
//		return 0;
	...
	if( nationPtr->ai_camp_count >= 7 - nationPtr->pref_military_development/30 )
		maxOutstandingLair = 2;		// if we already have this number of outstanding lairs, do not build new ones.
	else
	...
	for( i=town_array.size() ; i>0 ; i-- )
	{
		...
		if( curRating > bestRating )
		{
			...
			if( !nationPtr->is_action_exist( -1, -1, townPtr->loc_x1, townPtr->loc_y1,
				 ACTION_AI_BUILD_FIRM, FIRM_LAIR ) )						// 2-only check the reference location, ignore the action location
			{
...
void FirmLair::ai_build_lair(Place* placePtr)
{
	...
	nationPtr->add_action( buildXLoc, buildYLoc, placePtr->loc_x1, placePtr->loc_y1,
...
void FirmLair::think_recruit(int combatLevelDiff)
{
	...
	for( int firmId=FIRST_MONSTER_SPECIAL_FIRM ; firmId<=LAST_MONSTER_SPECIAL_FIRM ; firmId++ )
	{
		...
		if( firmInfo->nation_can_build(nation_recno, race_id, 0) &&		// 0-don't check cash
	...
	//--- if the commander has high leadership, have more soldiers under his training ---//
	...
	for( int i=0 ; i<linked_town_count ; i++ )
	{
			 ...
			 townPtr->resistance(nation_recno) >= MONSTER_COLLECT_TOWN_TRIBUTE_LOYALTY )
		{
			...
			if( nation_array[nation_recno]->ai_attack_target(townPtr->loc_x1,
				 townPtr->loc_y1, defenseCombatLevel, 0, 0, 0, firm_recno, 1 ) )
			{
				return;
			}
/*
			...
			if( firmCombatLevel > defenseCombatLevel * (100+ownNation->pref_force_projection*2/3) / 100 )
			{
				rc = 1;
			}
			else
